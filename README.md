# Cainsville Tree Service — Website

Static site for **Cainsville Tree Service**, Cainsville, Ontario. No framework, no build server, no
monthly hosting bill.

```
Cainsville-Tree-Trimming-Website-Content.md   <- every word on the site (source of truth)
...
This town runs `HEADER_VARIANT = "wordmark"` and `HERO_VARIANT = "editorial"`, set near the
...
**Copy** — edit `Cainsville-Tree-Trimming-Website-Content.md`, then `python build.py`.
...
The repo is set up for Cloudflare. The worker name is `cainsvilletreetrimming` and the
canonical host is `www.cainsvilletreetrimming.com`.
